...
class EventsController < ApplicationController
  ...
  before_action :authenticate_live, :only => [:live]
  ...
  def prebuy
    ...
    if @participation.payed
      ...
      @participation.event.product_address,
        :filename => @participation.event.product_address.split('/').last)
      else
  ...
  def book_conference
    authenticate_user!
    @profile = current_user.profile
    error=@profile.book_conference(@event,PriceModel.where(:id=>params[:price_model_id]).first,params[:cut_code])
    if error
      @error = error
    else
  ...
  def event_params
    params.require(:event).permit(:title, :sentence, :description, :summary, :begins, :venue, :category, :capacity, :price, :poster,:tag_list,:is_conference_like,:product_address, :officer_ids => [])
    # params.permit!
  end
