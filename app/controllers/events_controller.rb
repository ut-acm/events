class EventsController < ApplicationController
  ...
  def filter_by_year
    year = params[:year]
    @events=Event.where("begins > ?",Time.beginning_of_year(year.to_i)).where("begins > ?", Time.now).order(:begins)
    @archives=Event.where("begins > ?",Time.beginning_of_year(year.to_i)).where("begins < ?", Time.now).order(:begins)
